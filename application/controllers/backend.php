...
class Backend extends CI_Controller
{
	...
	function users()
	{
		$this->load->model('fx_auth/users', 'users');			
		...
		foreach ($_POST as $key => $value)
		{
			...
			if (substr($key, 0, 9) == 'checkbox_')
			{
				...
				else if (isset($_POST['reset_pass']))
				{
					...
					if ($user = $this->users->get_user_by_id($value, TRUE))
					{
						// Create new key, password and send email to user
						if ($this->fx_auth->forgot_password($user->username))
						{
							// Query once again, because the database is updated after calling forgot_password.
							$user = $this->users->get_user_by_id($value, TRUE);
													
							// Reset the password
							if ($this->fx_auth->reset_password($user->id, $user->new_password_key, '123456'))
							{							
	...
	{
		$this->load->model('fx_auth/user_temp', 'user_temp');
		...
		if ($this->input->post('activate'))
		{
			...
			foreach ($_POST as $key => $value)
			{
				...
				if (substr($key, 0, 9) == 'checkbox_')
				{
					...
					{
						// Activate user
						$this->fx_auth->activate($value, $query->row()->activation_key);
					}
				}				
			}
		}
